...
| # | Status | Item |
|---|--------|------|
| 5.4 | `todo` | **Reduce amleyg00 BIOS iGPU VRAM carve-out** (currently 24GiB of 32GiB, an ollama-era setting — OS sees only 7.5GiB and runs at ~57% memory). Plex transcoding needs a few hundred MB of VRAM; set carve-out to minimum/4GB in Framework BIOS (needs reboot + physical access). Afterward raise plex memory limit 2Gi → 4Gi (comment marker in `apps/plex/base/deployment.yaml`). Resize itself done in `8fc1680`: requests 1 CPU/1Gi, limits 8 CPU/2Gi. |
