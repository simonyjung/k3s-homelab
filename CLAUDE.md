...
- **Lint (the only CI gate):** `yamllint .` — run before committing. Config in `.yamllint` extends `default`; `line-length` max is 120 but only a warning. CI runs this on every push to `main` (paths-ignore `*.md`) and on a weekly schedule.
